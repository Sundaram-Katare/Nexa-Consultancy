...
import { ARCHIVE_ORG_CONFIG } from "./config";
import { ARCHIVE_ORG_SELECTORS } from "./selectors";

export class ArchiveOrgAdapter extends BaseSourceAdapter {
  public readonly id = ARCHIVE_ORG_CONFIG.SOURCE_ID;
  ...
  private async throttle(): Promise<void> {
    ...
    if (elapsed < ARCHIVE_ORG_CONFIG.MIN_REQUEST_DELAY_MS) {
      const waitTime = ARCHIVE_ORG_CONFIG.MIN_REQUEST_DELAY_MS - elapsed;
  ...
  private resolvePage(session: any): Page {
    ...
    throw new Error("Invalid session provided to ArchiveOrgAdapter: missing Playwright Page instance.");
  }
  ...
  protected doBuildSearchUrl(queryText: string, page: number = 1): string {
    ...
    return ARCHIVE_ORG_CONFIG.SEARCH_URL_TEMPLATE
      .replace("{query}", encodedQuery)
      .replace("{page}", page.toString());
  }

  /**
   * Extracts Archive.org Item Identifier from URL.
   * e.g. "https://archive.org/details/universityrecord1985" -> "universityrecord1985"
   */
  ...
  /**
   * Executes a search on Archive.org and returns structured SearchResultPage.
   */
  protected async doSearch(session: any, queryText: string): Promise<SearchResultPage> {
    ...
    console.log(`[ARCHIVE_ORG] Executing search: "${queryText}" (Page 1) -> ${searchUrl}`);

    await page.goto(searchUrl, {
      waitUntil: "domcontentloaded",
      timeout: ARCHIVE_ORG_CONFIG.NAVIGATION_TIMEOUT_MS,
    });
  ...
  protected async doGetNextPage(
    ...
    console.log(`[ARCHIVE_ORG] Navigating to page ${nextPage}: ${nextUrl}`);
  ...
  private async parseSearchPage(
    ...
    const noResults = await page.$(ARCHIVE_ORG_SELECTORS.SEARCH.NO_RESULTS);
    if (noResults) {
      console.log(`[ARCHIVE_ORG] No results found for query "${queryText}"`);
      return {
        results: [],
        hasNextPage: false,
        pageNumber,
    ...
    const rawCards = await page.evaluate((selectors) => {
      ...
      return elements.map((el) => {
        const anchor = el.tagName === "A" ? (el as HTMLAnchorElement) : el.querySelector("a[href*='/details/']");
        const href = anchor ? (anchor as HTMLAnchorElement).href : "";

        let title = "";
        const titleEl = el.querySelector(".ttl, h3, a.item-title, .item-ttl, span.title");
        if (titleEl) {
        ...
        const descEl = el.querySelector(".description, .item-description, p");
        const snippet = descEl ? descEl.textContent?.trim() : "";

        return { href, title, snippet };
      });
    }, {
      RESULT_CARDS: ARCHIVE_ORG_SELECTORS.SEARCH.RESULT_CARDS,
    });

    const results: SearchResult[] = [];
    for (const card of rawCards) {
      if (!card.href || !card.href.includes("/details/")) continue;
      ...
      results.push({
        ...
        titleRaw: card.title || "Untitled Archive.org Document",
    ...
    const hasNextBtn = await page.evaluate((selector) => {
      ...
      return nextEl !== null;
    }, ARCHIVE_ORG_SELECTORS.SEARCH.PAGINATION_NEXT);
  ...
  protected async doOpenDocument(session: any, result: SearchResult): Promise<void> {
    await this.throttle();
    const page = this.resolvePage(session);
    console.log(`[ARCHIVE_ORG] Opening document: ${result.canonicalUrl}`);
    ...
    if (response && response.status() >= 400) {
      throw new Error(`Failed to open Archive.org document: HTTP ${response.status()} ${response.statusText()}`);
    }

    const pageTitle = (await page.title()) || "";
    if (pageTitle.toLowerCase().includes("item cannot be found") || pageTitle.toLowerCase().includes("404")) {
      throw new Error(`Archive.org document not found (Page title: "${pageTitle}")`);
    }
  ...
  protected async doExtractMetadata(session: any): Promise<RawMetadata> {
    ...
    const metadata = await page.evaluate((selectors) => {
      ...
      return {
        title: title || "Untitled Archive.org Document",
        uploader: creator,
        description,
        uploadDate: date,
        documentType: "Historical / Institutional Document",
      ...
      TITLE: ARCHIVE_ORG_SELECTORS.DOCUMENT.TITLE,
      DESCRIPTION: ARCHIVE_ORG_SELECTORS.DOCUMENT.DESCRIPTION,
      CREATOR: ARCHIVE_ORG_SELECTORS.DOCUMENT.CREATOR,
      DATE: ARCHIVE_ORG_SELECTORS.DOCUMENT.DATE,
    });
  ...
  protected async doExtractContent(session: any): Promise<RawContent> {
    ...
    const textBlocks = await page.evaluate((selectors) => {
      ...
      TEXT_BLOCKS: ARCHIVE_ORG_SELECTORS.DOCUMENT.TEXT_BLOCKS,
      FALLBACK_CONTAINER: ARCHIVE_ORG_SELECTORS.DOCUMENT.FALLBACK_CONTAINER,
    });

    return {
      textBlocks,
      extractionMethod: "archive_org_ocr_dom",
